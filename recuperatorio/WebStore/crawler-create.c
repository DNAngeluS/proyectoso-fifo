...
int main(int argc, char **argv)
{
    ...
    if (argc != 6)
        rutinaDeError("Argumentos invalidos", config.log);
    ...
    if ((config.log = open("log-crawler-create.txt", O_CREAT | O_TRUNC | O_WRONLY, modeOpen)) < 0)
        rutinaDeError("Crear archivo Log", config.log);
    WriteLog(config.log, "Crawler Create", getpid(), 1, "Inicio de ejecucion", "INFO");

    /*Establecer conexion LDAP*/
    WriteLog(config.log, "Crawler Create", getpid(), 1, "Se establecera conexion LDAP", "INFO");
    if(establecerConexionLDAP(&ldap, config) < 0)
      rutinaDeError("No se pudo establecer la conexion LDAP.", config.log);
    WriteLog(config.log, "Crawler Create", getpid(), 1, "Conexion establecida OK", "INFOFIN");

    /*Se asigna un handler a la señal, si no se bloqueo su captura antes*/
    if (signal(SIGUSR1, signalHandler) == SIG_ERR)
        rutinaDeError("signal", config.log);
    ...
    WriteLog(config.log, "Crawler Create", getpid(), 1, "Se enviara instanciacion de Crawler al WebServer conocido", "INFO");
    if (EnviarCrawler(config.ipWebServer, config.puertoWebServer, &modeConocido) < 0)
	{
            WriteLog(config.log, "Crawler Create", getpid(), 1, "Enviado primer Crawler", "ERROR");
            rutinaDeError("Enviando primer Crawler", config.log);
	}
	else
            WriteLog(config.log, "Crawler Create", getpid(), 1, "Enviado OK", "INFOFIN");
    ...
    ldapActualizarHost(&ldap, ipPuertoConocido, time(NULL), ALTA, config);
    ...
    while (1)
    {
        ...
        WriteLog(config.log, "Crawler Create", getpid(), 1, text, "INFOFIN");
        ...
        WriteLog(config.log, "Crawler Create", getpid(), 1, "Se obtendran los hosts del directorio de hosts", "INFO");
        if (ldapObtenerHosts(&ldap, &hosts, &maxHosts, config) < 0)
        {
            WriteLog(config.log, "Crawler Create", getpid(), 1, "Error", "ERROR");
            continue;
        }
        WriteLog(config.log, "Crawler Create", getpid(), 1, "Obtenidos OK", "INFOFIN");

        WriteLog(config.log, "Crawler Create", getpid(), 1, "Se dispararan Crawlers a hosts desactualizados", "INFOFIN");
        ...
        /*Para cada hosts en la tabla*/
        {
            ...
            /*Si el tiempo que paso desde la ultima migracion es mayor al configurado*/
            {
                ...
                WriteLog(config.log, "Crawler Create", getpid(), 1, text, "INFO");
                if (EnviarCrawler(hosts[i].hostIP, hosts[i].hostPort, &mode) < 0)
                    WriteLog(config.log, "Crawler Create", getpid(), 1, "Error al enviar instanciacion", "ERROR");
                else
                {
                    WriteLog(config.log, "Crawler Create", getpid(), 1, "Instanciacion enviada OK", "INFOFIN");
                    ...
                    if (mode == IRC_CRAWLER_OK)
                    {
                            ...
                            WriteLog(config.log, "Crawler Create", getpid(), 1, "Crawler a migrado satisfactoriamente", "INFOFIN");
                            sprintf(ipPuerto, "%s:%d", inet_ntoa(*(struct in_addr *) &(hosts[i].hostIP)), ntohs(hosts[i].hostPort));

                            /*Se actualiza el timestamp del host al que se le envio el Crawler*/
                            WriteLog(config.log, "Crawler Create", getpid(), 1, "Se actualizara Unix Timestamp del host", "INFO");
                            if (ldapActualizarHost(&ldap, ipPuerto, time(NULL), MODIFICACION, config) < 0)
                                WriteLog(config.log, "Crawler Create", getpid(), 1, "Error al actualizar Unix Timestamp", "ERROR");
                            else
                                WriteLog(config.log, "Crawler Create", getpid(), 1, "Unix Timestamp actualizado OK", "INFOFIN");
                    }
                    else if (mode == IRC_CRAWLER_FAIL)
                    {
                        WriteLog(config.log, "Crawler Create", getpid(), 1, "Crawler a sido rechazado", "INFOFIN");
                    }
                }
            }
            else
        ...
        {
            WriteLog(config.log, "Crawler Create", getpid(), 1, "No se encontraron hosts en el directorio de expiracion de hosts", "INFOFIN");
            putchar('\n');
        }
    }
